...
selected = option_menu(
        menu_title = None,
        options=["Home","Business card","Display","About"],
        icons=["house-fill","database-fill","database-fill","exclamation-lg"],
        default_index=0,
...
#------------------------------------------------------Display-----------------------------------------------------
if selected == 'Display':
        mycursor.execute("SELECT * FROM card_details ORDER BY card_holder;")
        df = pd.DataFrame(mycursor.fetchall(),columns=['Company Name', 'Card Holder', 'Designation','Mobile Number','Email','Website','Area','City','State','Pincode'])
        df.index += 1
        st.dataframe(df)
